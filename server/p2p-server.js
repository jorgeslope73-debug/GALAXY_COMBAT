...
wss.on('connection',ws=>{
  ...
  ws.on('message',async raw=>{
    ...
    if(m.t==='fallback-ctrl'){
      ...
      if(x.i!==0&&host)send(host.ws,{t:'fallback-ctrl',from:x.i,turn:Number(m.turn)||0,thrust:!!m.thrust,fire:!!m.fire});
      return;
    }
    if(m.t==='fallback-state'){
      if(x.i===0&&m.state){
        for(const p of r.players)if(p.i!==0)send(p.ws,{t:'fallback-state',state:m.state});
      }
      return;
    }
    if(m.t==='fallback-event'){
      if(x.i===0&&m.event){
        for(const p of r.players)if(p.i!==0)send(p.ws,{t:'fallback-event',event:m.event});
      }
      return;
    }
    if(m.t==='fallback-action'){
      const host=r.players.find(p=>p.i===0);
      if(x.i!==0&&host)send(host.ws,{t:'fallback-action',from:x.i,action:String(m.action||'')});
      return;
    }
